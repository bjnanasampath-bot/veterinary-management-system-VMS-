      ...
      <div className="card">
        ...
        <table className="w-full text-sm mb-6">
          ...
          <tbody className="divide-y divide-gray-50">
            {(bill.items || []).map((item, i) => (
              <tr key={i}><td className="py-2.5">{item.description}</td><td className="py-2.5 text-center capitalize text-gray-500">{item.item_type}</td><td className="py-2.5 text-right">{item.quantity}</td><td className="py-2.5 text-right">₹{item.unit_price}</td><td className="py-2.5 text-right font-medium">₹{item.total_price}</td></tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="flex justify-end">
          <div className="w-56 space-y-1.5 text-sm">
            ...
            <div className="flex justify-between font-bold text-gray-900 text-base pt-2 border-t"><span>Total</span><span>₹{bill.total_amount}</span></div>
            <div className="flex justify-between text-green-600"><span>Paid</span><span>₹{bill.paid_amount}</span></div>
            <div className="flex justify-between text-red-500 font-semibold"><span>Due</span><span>₹{bill.due_amount}</span></div>
          </div>
        </div>
      </div>
